...
export const updateAssistant = async (req, res) => {
    try {
        ...
        return res.status(400).json({
            message: "update assistatnt error",
        });
    }
};
...
export const askToAssistant = async (req, res) => {
    try {
        ...
        const result = await geminiResponse(command, name, assistantName);
        ...
        if (error.code === 429) {
            return res.status(429).json({
                response:
                    error.message ||
                    "I'm currently at my usage limit. Please try again in a minute.",
